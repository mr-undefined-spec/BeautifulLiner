...
class SplitRangeCohandler:
    ...
    @staticmethod
    def create_split_ranges(curve_orientations, index_offset):
        ranges = []
        ...
        for i, value in enumerate(curve_orientations):
            ...
            # end if
        # end for
        ranges.append((pre_index + index_offset, len(curve_orientations) + index_offset))
        return ranges
    # end create_split_ranges

    @staticmethod
    def find_split_point_by_angle(points, start_index, end_index, angle_threshold_deg):
        if len(points) < 3:
            return -1
        # end if
        ...
        for i in range(start_index + 1, end_index - 2):
            p = points[i]
            dist = GeometryCohandler.get_distance_point_to_segment(start, p, end)
            if dist > max_dist:
                max_dist = dist
                max_index = i
            # end if
        ...
        angle_rad = GeometryCohandler.calculate_angle_between_vectors(vec1, vec2)
