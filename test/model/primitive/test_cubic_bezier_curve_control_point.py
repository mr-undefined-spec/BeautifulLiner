...
class TestCubicBezierCurveControlPoint(unittest.TestCase):
    ...
    def test_shapely_integration(self):
        # 内部キャッシュが正しくMultiPointとして機能しているか
        mp = self.bezier_ctrl_p.shapely
        self.assertEqual(mp.geom_type, "MultiPoint")
        self.assertEqual(len(mp.geoms), 4)
        
        # 同一インスタンスキャッシュの確認
        self.assertIs(mp, self.bezier_ctrl_p.shapely)
    #end
    ...
    def test_raise_error_with_set_p0_to_p3_as_int(self):
        ...
        with self.assertRaises(TypeError) as e:
            CubicBezierCurveControlPoint(0, p, p, p) # type: ignore
        self.assertEqual(e.exception.args[0], 'p0 must be Point')

        with self.assertRaises(TypeError) as e:
            CubicBezierCurveControlPoint(p, 0, p, p) # type: ignore
        self.assertEqual(e.exception.args[0], 'p1 must be Point')

        with self.assertRaises(TypeError) as e:
            CubicBezierCurveControlPoint(p, p, 0, p) # type: ignore
        self.assertEqual(e.exception.args[0], 'p2 must be Point')

        with self.assertRaises(TypeError) as e:
            CubicBezierCurveControlPoint(p, p, p, 0) # type: ignore
